...
if(isset($_POST['generate'])){
  ...
  // page table
  $pdf->AddPage();
  $pdf->Cell(0,8,'Companies With Existing MOA',0,1,'C');
  $pdf->AddCol('company_name',100,'Company');
  $pdf->AddCol('moa',30,'Starting Date');
  $pdf->AddCol('exp_date',30,'Expiration Date');
  $pdf->AddCol('moa_status',30,'Validity');
  $prop = array('HeaderColor'=>array(255,150,100),
              'color1'=>array(210,245,255),
              'color2'=>array(255,255,210),
              'padding'=>2);

  $pdf->Table($conn,'select company_name, moa, exp_date, moa_status from companies_1 where moa != " " order by exp_date desc',$prop);
  $pdf->Output();
}
...
<body>
    ...
    <div>
      ...
      <h2 style="color:#a00303; text-align:center; margin-top:100px;">Partner Companies With Existing MOA</h2>
    </div>
    ...
    <div>
      <form action="index.php#services" method="POST">
        ...
        <button type="submit" id="csvs" id="exportButton" class = "button2" style="float:right; margin-right:10px;" onclick="exportToCsv('CompaniesWithMoa.csv', ',', [0, 7],
        ['Companies With Existing MOA'])" title="Generate Report as CSV" data-rel="tooltip"><i class="fa fa-download"></i>CSV&nbsp;&nbsp;&nbsp;</button>
...
<table id="myTable">
  ...
  <?php
    $query = "SELECT *, DATE_FORMAT(moa, '%M %d, %Y') as start_date, DATE_FORMAT(exp_date, '%M %d, %Y') as end_date FROM companies_1 WHERE moa != ' '";
